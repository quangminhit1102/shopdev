...
erDiagram
    shopdev_user {
        int user_id PK "AUTO_INCREMENT, user id"
        varchar user_name "255, user name"
        varchar user_email "255, email user"
    }

    shopdev_role {
        int role_id PK "role id"
        varchar role_name "255, role name"
        varchar role_description "255, role description"
    }

    shopdev_menu {
        int menu_id PK "AUTO_INCREMENT, menu_id"
        varchar menu_name "255, name menu"
        varchar menu_pid "255, parent menu id"
        varchar menu_path "255, path"
    }
    ...
    shopdev_role_menu {
        int role_id PK,FK "role id"
        int menu_id PK,FK "menu id"
    }

    %% Relationships - Many to Many through junction tables
    shopdev_user ||--o{ shopdev_user_role : "assigned"
    shopdev_role ||--o{ shopdev_user_role : "has"
    shopdev_role ||--o{ shopdev_role_menu : "accesses"
    shopdev_menu ||--o{ shopdev_role_menu : "available_to"

    %% Sample Data Notes
    %% Users: admin, shop, user
    %% Roles: admin(CRUD), shop(RUC), user(R)
    %% Menus: Dong ho, may tinh, thoi trang nam
```
